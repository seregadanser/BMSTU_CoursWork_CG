...
namespace cours_m2G
{
    ...
    class RayTraiserPool : RayTraiser
    {
        ...
        private Color RayT(Model model, MatrixCoord3D D, MatrixCoord3D position)
        {
            ...
            PolygonComponent closest = null;
            double closest_t = double.MaxValue;
            List<PolygonComponent> pp = model.Polygons.CC;
            {
               for(int i = 0; i < pp.Count; i++)
                {
                     //  if (MatrixCoord3D.scalar(pp[i].Normal, cam.Direction) > 0)
                    {
                        MatrixCoord3D tt = GetTimeAndUvCoord(position, D, pp[i].Points[0].Coords, pp[i].Points[1].Coords, pp[i].Points[2].Coords);
                        if (tt != null)
                        {
                            if (tt.X < closest_t && tt.X > 1)
                            {
